...
def get_args(instr=''):
    ...
    if instr == '':
        ...
        return parser.parse_args(instr)
...
#    pprint.pprint(result['artists'])
    ...
    if WebOutput != None:
        WebOutput.add_to_result((f"<td class='track-number'>{i}</td>\n").encode("utf-8"))
        WebOutput.add_to_result((f"<td class='play-count'>{playcount}</td>\n").encode("utf-8"))
        if track_link != None:
            WebOutput.add_to_result((f"<td class='track-name'><a href='{track_link}'><i class='far fa-play-circle'></i></a>{track_name}</td>\n").encode("utf-8"))
        else:
            WebOutput.add_to_result((f"<td class='track-name'>{track_name}</td>\n").encode("utf-8"))
        WebOutput.add_to_result((f"<td class='artist'>{artists_with_links}</td>\n").encode("utf-8"))
        WebOutput.add_to_result((f"<td class='album'><a href='{album_link}'><i class='fas fa-link'></i></a>{my_album_link}</td>\n").encode("utf-8"))
        if len(playlists) > 0:
            WebOutput.add_to_result((f"<td class='playlist-present'>").encode("utf-8"))
        else:
            WebOutput.add_to_result((f"<td class='playlist-absent'>").encode("utf-8"))
        for playlist in playlists:
            WebOutput.add_to_result((f"{playlist}<br/>").encode("utf-8"))
        WebOutput.add_to_result((f"</td>\n").encode("utf-8"))
        WebOutput.add_to_result((f"<td class='track-uri'>{result['uri']}</td>\n").encode("utf-8"))
    else:
        playlist = ""
        if len(playlists) > 0:
...
def webinit():
    ...
    if SpotifyAPI == None:
        myArgs = {}
        myArgs['last-fm'] = True
        myArgs['bandcamp'] = True
        myArgs['everynoise'] = True
...
def webfinish():
    return WebOutput.Result

def do_GET_error():
    abort(404)

def addCSS():
    WebOutput.add_to_result((f"<link rel='stylesheet' href='{url_for('static', filename='spotify-playlist.css')}'>").encode("utf-8"))

def addCSS_from_file(file_name):
    ...
    try:
        with open(file_name,'r') as cssfile:
            for line in cssfile:
                WebOutput.add_to_result(line.encode("utf-8"))
    except:
...
def addAlbum_Container_Start(album):
    WebOutput.add_to_result(b"<div class='album-container'>")
    WebOutput.add_to_result((f"<a id='{getAlbumTarget(album)}'></a>").encode("utf-8"))

def addAlbum_Header_Start():
    WebOutput.add_to_result(b"<div class='album-header'>")

def addAlbum_Header_End():
    WebOutput.add_to_result(b"</div>")

def addAlbum_SubContainer_Start(album):
    WebOutput.add_to_result(b"<div class='album-subcontainer'>")

def addAlbum_SubContainer_End():
    WebOutput.add_to_result(b"</div>")

def addAlbum_Container_End():
    WebOutput.add_to_result(b"</div>")

    WebOutput.add_to_result(b"<div class='clear'/>")

def addAlbum_Art(url):
    WebOutput.add_to_result(b"<div class='album-image'>")
    if url != "":
        WebOutput.add_to_result((f"<img src='{url}' width='100%' />").encode("utf-8"))
    WebOutput.add_to_result(b"</div>")

def addAlbum_Details(release_date,genres,show_top_link=None):
    WebOutput.add_to_result(b"<div class='album-details'>")
    if show_top_link != None:
        WebOutput.add_to_result(show_top_link)
    if release_date != "":
        WebOutput.add_to_result((f"<div>Release date: {release_date}</div>\n").encode("utf-8"))
    goodGenres = []
    badGenres = []
    untaggedGenres = []
    if len(genres) > 0 and genres[0].strip() != "":
        WebOutput.add_to_result((f"<div>Genre: ").encode("utf-8"))
        for genre in genres:
            if genre in GenresGood:
                WebOutput.add_to_result((f"<span class='genre-good'>{genre}</span><br/>").encode("utf-8"))
                goodGenres.append(genre)
            elif genre in GenresBad:
                WebOutput.add_to_result((f"<span class='genre-bad'>{genre}</span><br/>").encode("utf-8"))
                badGenres.append(genre)
            else:
                WebOutput.add_to_result((f"<span class='genre-untagged'>{genre}</span><br/>").encode("utf-8"))
...
def addAlbum_Actions(album,goodGenres,badGenres,untaggedGenres):
    WebOutput.add_to_result(b"<div class='album-actions'>")
    ...
    if 'uri' in album:
        ...
        if 'watchlist_entry' in album:
            WebOutput.add_to_result(b'<form action="/remove_album_from_watchlist">')
            WebOutput.add_to_result(b'<label for="remove_album_from_watchlist">Remove from watchlist:</label>')
            WebOutput.add_to_result(b"<input type='submit' value='Submit'>")
            WebOutput.add_to_result((f"<input type='hidden' name='app' value='spotify'</input>\n").encode("utf-8"))
            WebOutput.add_to_result((f"<input type='hidden' name='artist' value='{quote_plus(album['watchlist_entry']['artist'])}'</input>\n").encode("utf-8"))
            WebOutput.add_to_result((f"<input type='hidden' name='album' value='{quote_plus(album['watchlist_entry']['album'])}'</input>\n").encode("utf-8"))
            if 'return_target' in album:
                WebOutput.add_to_result((f"<input type='hidden' name='return_target' value='{quote_plus(album['return_target'])}'</input>\n").encode("utf-8"))
            WebOutput.add_to_result((f"<input type='hidden' name='return' value='{request.path}'</input>\n").encode("utf-8"))
            WebOutput.add_to_result(b'</form>')
            WebOutput.add_to_result((f"<form action='/add_album_to_playlist'>\n").encode("utf-8"))
            WebOutput.add_to_result(b'<label for="playlist">Move from watchlist to playlist:</label>')
        else:
            WebOutput.add_to_result((f"<form action='/add_album_to_playlist'>\n").encode("utf-8"))
            WebOutput.add_to_result(b'<label for="playlist">Add album to playlist:</label>')
        WebOutput.add_to_result(b'<select name="playlist" id="playlist">\n')
        for playlist_id in PlaylistDetailsCache:
            ...
            if Args.server_default_playlist == entry['name']:
                selected = "selected"
            WebOutput.add_to_result((f"<option value='{playlist_id}' {selected}>{entry['name']}</option>\n").encode("utf-8"))
        WebOutput.add_to_result(b"</select>\n")
        WebOutput.add_to_result(b"<input type='submit' value='Submit'>")
        WebOutput.add_to_result((f"<input type='hidden' name='app' value='spotify'</input>\n").encode("utf-8"))
        WebOutput.add_to_result((f"<input type='hidden' name='album' value='{album['id']}'</input>\n").encode("utf-8"))
        if 'watchlist_entry' in album:
            WebOutput.add_to_result((f"<input type='hidden' name='watchlist_artist' value='{quote_plus(album['watchlist_entry']['artist'])}'</input>\n").encode("utf-8"))
            WebOutput.add_to_result((f"<input type='hidden' name='watchlist_album' value='{quote_plus(album['watchlist_entry']['album'])}'</input>\n").encode("utf-8"))
        if 'return_target' in album:
            WebOutput.add_to_result((f"<input type='hidden' name='return_target' value='{quote_plus(album['return_target'])}'</input>\n").encode("utf-8"))
        WebOutput.add_to_result((f"<input type='hidden' name='return' value='{request.path}'</input>\n").encode("utf-8"))
        WebOutput.add_to_result(b"</form>")
        ...
        if 'bandcamp_link' in album and album['bandcamp_link'] != "":
            ...
            WebOutput.add_to_result(bandcamplink.encode("utf-8"))
        else:
        ...
        if in_watchlist(entry):
            ...
            WebOutput.add_to_result(b"<input type='submit' value='Submit'>")
            WebOutput.add_to_result((f"<input type='hidden' name='app' value='spotify'</input>\n").encode("utf-8"))
            WebOutput.add_to_result((f"<input type='hidden' name='artist' value='{quote_plus(entry['artist'])}'</input>\n").encode("utf-8"))
            WebOutput.add_to_result((f"<input type='hidden' name='album' value='{quote_plus(entry['album'])}'</input>\n").encode("utf-8"))
            if 'return_target' in album:
                WebOutput.add_to_result((f"<input type='hidden' name='return_target' value='{quote_plus(album['return_target'])}'</input>\n").encode("utf-8"))
            WebOutput.add_to_result((f"<input type='hidden' name='return' value='{request.path}'</input>\n").encode("utf-8"))
            WebOutput.add_to_result(b'</form>')
            ...
            WebOutput.add_to_result(b'<form action="/add_album_to_watchlist">')
            WebOutput.add_to_result(b'<label for="add_album_to_watchlist">Add to watchlist:</label>')
            WebOutput.add_to_result(b"<input type='submit' value='Submit'>")
            WebOutput.add_to_result((f"<input type='hidden' name='app' value='spotify'</input>\n").encode("utf-8"))
            WebOutput.add_to_result((f"<input type='hidden' name='artist' value='{quote_plus(album['artist'])}'</input>\n").encode("utf-8"))
            WebOutput.add_to_result((f"<input type='hidden' name='album' value='{quote_plus(album['name'])}'</input>\n").encode("utf-8"))
            WebOutput.add_to_result((f"<input type='hidden' name='release_date' value='{album['release_date']}'</input>\n").encode("utf-8"))
            WebOutput.add_to_result((f"<input type='hidden' name='genre' value='{genres}'</input>\n").encode("utf-8"))
            WebOutput.add_to_result((f"<input type='hidden' name='art' value='{album['art']}'</input>\n").encode("utf-8"))
            WebOutput.add_to_result((f"<input type='hidden' name='return' value='{request.path}'</input>\n").encode("utf-8"))
            WebOutput.add_to_result(b'</form>')

    if len(badGenres) + len(untaggedGenres) > 0:
        WebOutput.add_to_result((f"<form action='/tag_genre'>\n").encode("utf-8"))
        WebOutput.add_to_result(b'<label for="genre_good">Add genre to liked:</label>')
        WebOutput.add_to_result(b'<select name="tag_genre_good" id="tag_genre_good">\n')
        for genre in untaggedGenres:
            WebOutput.add_to_result((f"<option value='{genre}'>{genre}</option>\n").encode("utf-8"))
        for genre in badGenres:
            WebOutput.add_to_result((f"<option value='{genre}'>{genre}</option>\n").encode("utf-8"))
        WebOutput.add_to_result(b"</select>\n")
        WebOutput.add_to_result(b"<input type='submit' value='Submit'>")
        WebOutput.add_to_result((f"<input type='hidden' name='app' value='spotify'</input>\n").encode("utf-8"))
        WebOutput.add_to_result((f"<input type='hidden' name='return_target' value='{getAlbumTarget(album)}'</input>\n").encode("utf-8"))
        WebOutput.add_to_result((f"<input type='hidden' name='return' value='{request.path}'</input>\n").encode("utf-8"))
        WebOutput.add_to_result(b"</form>")

    if len(badGenres) + len(goodGenres) > 0:
        WebOutput.add_to_result((f"<form action='/tag_genre'>\n").encode("utf-8"))
        WebOutput.add_to_result(b'<label for="genre_untagged">Remove tag from genre:</label>')
        WebOutput.add_to_result(b'<select name="tag_genre_untagged" id="tag_genre_untagged">\n')
    ...
    if len(goodGenres) + len(untaggedGenres) > 0:
        WebOutput.add_to_result((f"<form action='/tag_genre'>\n").encode("utf-8"))
        WebOutput.add_to_result(b'<label for="genre_bad">Add genre to disliked:</label>')
        WebOutput.add_to_result(b'<select name="tag_genre_bad" id="tag_genre_bad">\n')
        for genre in untaggedGenres:
            WebOutput.add_to_result((f"<option value='{genre}'>{genre}</option>\n").encode("utf-8"))
        for genre in goodGenres:
            WebOutput.add_to_result((f"<option value='{genre}'>{genre}</option>\n").encode("utf-8"))
...
def addAlbum_Tracklist(album):
    WebOutput.add_to_result(b"<div class='album-tracklist'>")
    WebOutput.add_to_result(b"<table>")
    for i,track in enumerate(SpotifyAPI.album_tracks(album['id'])['items']):
        WebOutput.add_to_result(b"<tr>")
...
def addMissingAlbum(album):
    ...
    addAlbum_Container_Start(album)

    if 'art' in album:
        addAlbum_Art(album['art'])

    addAlbum_SubContainer_Start(album)

    addAlbum_Header_Start()
    goodGenres, badGenres, untaggedGenres = addAlbum_Details(album['release_date'],album['genres'])
    addAlbum_Actions(album,goodGenres,badGenres,untaggedGenres)
    addAlbum_Header_End()
    ...
    addAlbum_SubContainer_End()

    addAlbum_Container_End()

def chooseAlbumArt(images,target_width):
    best_error = 0
    best_url = ""
    for image in images:
...
def addAlbum(album,release_date='',genres=[],showname=False,showartist=False):
    show_top_link = None
    if showname:
        if showartist:
    ...
    if len(album['images']) > 0:
        addAlbum_Art(chooseAlbumArt(album['images'],300))

    addAlbum_SubContainer_Start(album)

    addAlbum_Header_Start()
    goodGenres, badGenres, untaggedGenres = addAlbum_Details(release_date,genres)
    addAlbum_Actions(album,goodGenres,badGenres,untaggedGenres)
    addAlbum_Header_End()

    addAlbum_Tracklist(album)

    addAlbum_SubContainer_End()

    addAlbum_Container_End()

def filter_results(results,artist,album):
#        return [results[0]]
...
def getResults(track_name,artist,album,free_text=''):
    results = get_search_exact('*',artist,album,free_text)
    if free_text == '':
        results = filter_results(results,artist,album);
...
def addRedirect(params,message):
    fragment = ""
    if 'return_target' in params:
        fragment = f"#{params['return_target']}"
    elif 'album' in params:
        fragment = f"#album_{params['album']}"
    target = "/"
    if 'return' in params:
        target = params['return']

    redirect_url = f"{target}?app=spotify&message={message}{fragment}"
    print(f"Redirecting to {redirect_url}")
    return redirect(redirect_url)    

def addTopOfPage(subtitle=""):
#    self.send_response(200)
#    self.send_header('Content-type', 'text/html; charset=UTF-8')
#    self.end_headers()
    WebOutput.add_to_result(b'<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01//EN" "http://www.w3.org/TR/html4/strict.dtd">\n')
    WebOutput.add_to_result(b"<html>\n")
    WebOutput.add_to_result(b"<head>\n")
    addCSS()
    WebOutput.add_to_result(b"</head>\n")

    WebOutput.add_to_result(b"<body>\n")
    if len(subtitle) > 0:
        WebOutput.add_to_result((f"<h1><a href='/?app=spotify'>Spotify Toolbox</a> - {subtitle}</h1>\n").encode("utf-8"))
    else:
        WebOutput.add_to_result(b"<h1><a href='/?app=spotify'>Spotify Toolbox</a></h1>\n")

@app.route('/')
def do_GET_main_page():
    webinit()
    addTopOfPage()

    WebOutput.add_to_result(b"<ul>\n")
    WebOutput.add_to_result(b'<li><a href="/search?app=spotify">Search</a></li>\n')
    WebOutput.add_to_result(b'<li><a href="/releases?app=spotify">MetalStorm New Releases</a></li>\n')
    WebOutput.add_to_result(b'<li><a href="/playlists?app=spotify">Playlists</a></li>\n')
    WebOutput.add_to_result(b'<li><a href="/current?app=spotify">Currently Playing</a></li>\n')
    WebOutput.add_to_result(b'<li><a href="/recent?app=spotify">Recent Tracks</a></li>\n')
    WebOutput.add_to_result(b'<li><a href="/watchlist?app=spotify">Watchlist</a></li>\n')
    WebOutput.add_to_result(b'<li><a href="/watchlist_match_only?app=spotify">Watchlist (only matched)</a></li>\n')
    WebOutput.add_to_result(b'<li><a href="/watchlist_no_match?app=spotify">Watchlist (only unmatched)</a></li>\n')
    WebOutput.add_to_result(b"</ul>\n")
  ...
  <div class="search-container">
    ...
    if 'search' in params:
        ...
        if " - " in search_term:
            ...
            WebOutput.add_to_result((f"<h2>Results for {artist},{album}</h2>\n").encode("utf-8"))
            results = getResults('*',artist,album)

            for album in results:
                WebOutput.add_to_result(b"<hr/>")
                addAlbum(album)
        else:
            data = search_term

            WebOutput.add_to_result((f"<h2>Results for {search_term}</h2>\n").encode("utf-8"))
...
def do_GET_show_album():
    params = request.args

    if 'albumid' in params:
        album = read_SpotifyAlbum(params['albumid'])
        addTopOfPage(f"Album - {album['name']}")

    addAlbum(album, showname=True, showartist=True)

    WebOutput.add_to_result(b"</body>\n")
    WebOutput.add_to_result(b"</html>")

def do_GET_show_artist():
    params = request.args

    if 'artistid' in params:
        artist = SpotifyAPI.artist(params['artistid'])
        addTopOfPage(f"Artist - {artist['name']}")

        if Args.everynoise:
            everynoisegenres = get_everynoise_genres(params['artistid'])
            if everynoisegenres != None:
                aslist = ",".join(everynoisegenres)
                aslink = f"<h3>{aslist}</h3>\n".encode("utf-8")
                WebOutput.add_to_result(aslink)

        albums = []
        results = SpotifyAPI.artist_albums(params['artistid'], album_type='album')
...
def do_GET_recent():
    ...
    addTopOfPage("Recent Tracks")

    params = request.args

    showMessage(params)

    WebOutput.add_to_result(b"<div class='playlist-view'>")
    WebOutput.add_to_result(b"<div class='album-tracklist'>")
    WebOutput.add_to_result(b"<table>\n")
    i = 0
    tracks = SpotifyAPI.current_user_recently_played()
...
def do_GET_current():
    webinit()
    addTopOfPage("Currently Playing")
...
#        print(json.dumps(track_entry,indent=4))
    if track_entry != None and track_entry['item'] != None:
        ...
        WebOutput.add_to_result(b"<div class='playlist-view'><div class='album-tracklist'>")
        WebOutput.add_to_result(b"<table>")
        WebOutput.add_to_result(b"<tr>")
        print_track(track,track['track_number'],track['album'])
        WebOutput.add_to_result(b"</tr>")
        WebOutput.add_to_result(b"</table>")
        WebOutput.add_to_result(b"</div></div>")
        ...
        if track_entry['context'] and track_entry['context']['type'] and track_entry['context']['type'] == 'playlist':
            ...
            WebOutput.add_to_result((f"Playing from playlist <a href='/playlists?app=spotify&playlist={playlist['id']}'>{playlist['name']}</a>").encode("utf-8"))
...
def do_GET_releases():
    webinit()
    addTopOfPage("MetalStorm New Releases")
...
# Do cache it, but refresh more frequently
    ...
    for release in tracks_to_import + tracks_to_import2:
        if release['release_date'] != last_release_date:
            WebOutput.add_to_result(b"<hr/>")
            WebOutput.add_to_result((f"<h2>Releases for {release['release_date']}</h2>\n").encode("utf-8"))
            WebOutput.add_to_result(b"<hr/>")
            last_release_date = release['release_date']
        WebOutput.add_to_result((f"<h3>Results for {release['artist']},{release['album']}</h3>\n").encode("utf-8"))
        results = getResults('*',release['artist'],release['album'])
        if len(results) == 0:
            addMissingAlbum(release)
        else:
            for album in results:
                addAlbum(album,release['release_date'],release['genres'])
...
@app.route('/watchlist_match_only')
def do_GET_watchlist_match_only():
    return do_GET_watchlist(True,True)

@app.route('/watchlist_no_match')
def do_GET_watchlist_no_match():
    return do_GET_watchlist(True,False)
    
@app.route('/watchlist')
def do_GET_watchlist(filter=False,matched_only=True):
    webinit()
    addTopOfPage("Watchlist")
    ...
    for release in Watchlist:
        ...
        if len(results) == 0:
            if not (filter and matched_only):
                WebOutput.add_to_result(b"<hr/>")
                WebOutput.add_to_result((f"<h2>Results for {release['artist']},{release['album']}</h2>\n").encode("utf-8"))
...
def do_GET_add_album_to_playlist():
    ...
    if 'album' in params and 'playlist' in params:
        album = read_SpotifyAlbum(params['album']) #SpotifyAPI.album(params['album'][0])
        playlist = SpotifyAPI.playlist(params['playlist'])
        ...
        if 'watchlist_artist' in params and 'watchlist_album' in params:
            artist = unquote_plus( params['watchlist_artist'] )
            album = unquote_plus( params['watchlist_album'] )
...
def do_GET_add_album_to_watchlist():
    ...
    if 'album' in params and 'artist' in params:
        entry = { 'track': '*', 'artist': unquote_plus(params['artist']), 'album': unquote_plus(params['album']), 'release_date': "", 'genres': {}, 'art': "" }
        if 'release_date' in params:
            entry['release_date'] = params['release_date']
        if 'genre' in params:
            entry['genres'] = params['genre'].split(",")
        if 'art' in params:
...
def do_GET_playlists_add_header():
    ...
    playlist_selected = ''
    if 'playlist' in params:
        playlist_selected = params['playlist']

    addTopOfPage("Playlists")

    showMessage(params)

    WebOutput.add_to_result((f"<form action='/playlists'>\n").encode("utf-8"))
    WebOutput.add_to_result(b'<label for="playlist">Select user playlist:</label>')
    WebOutput.add_to_result(b'<select name="playlist" id="playlist">\n')
    for playlist_id in PlaylistDetailsCache:
        entry = PlaylistDetailsCache[playlist_id]
        if entry['owner_id'] != user_id:
    ...
    WebOutput.add_to_result(b"</select>\n")
    WebOutput.add_to_result(b"<input type='submit' value='Submit'>")
    WebOutput.add_to_result((f"<input type='hidden' name='app' value='spotify'>\n").encode("utf-8"))
    WebOutput.add_to_result(b"</form>\n")

    WebOutput.add_to_result((f"<form action='/playlists'>\n").encode("utf-8"))
    WebOutput.add_to_result(b'<label for="playlist">Select system playlist:</label>')
    WebOutput.add_to_result(b'<select name="playlist" id="playlist">\n')
    for playlist_id in PlaylistDetailsCache:
        entry = PlaylistDetailsCache[playlist_id]
...
def do_GET_playlists():
    ...
    playlist = None
    if 'playlist' in params:
        print(f"Found playlist in url: {params['playlist']}")
        playlist = SpotifyAPI.playlist(params['playlist'])
    if playlist == None:
        do_GET_playlists_add_header()
        return webfinish()

    if 'action' in params:
        action = params['action']
        if action == 'delete-first-N' and 'firstN' in params:
            firstN = (int)(params['firstN'])
#                print(f"Would delete first {firstN} albums from playlist {playlist['name']}")
            ...
            old_playlist = SpotifyAPI.playlist(params['playlist'])
            track_ids = []
    ...
    if playlist['owner']['id'] == user_id:
        WebOutput.add_to_result((f"<form action='/playlists'>\n").encode("utf-8"))
        WebOutput.add_to_result(b'<label for="playlist">Delete first albums from playlist:</label>')
        WebOutput.add_to_result((f"<input type='number' name='firstN' id='firstN' min='1'>\n").encode("utf-8"))
        WebOutput.add_to_result((f"<input type='hidden' name='app' value='spotify'>\n").encode("utf-8"))
        WebOutput.add_to_result((f"<input type='hidden' name='action' value='delete-first-N'>\n").encode("utf-8"))
        WebOutput.add_to_result((f"<input type='hidden' name='playlist' value='{playlist['id']}'>\n").encode("utf-8"))
#                WebOutput.add_to_result((f"<input type='hidden' name='return' value='{request.path}'>\n").encode("utf-8"))
        WebOutput.add_to_result(b"<input type='submit' value='Submit'>")
        WebOutput.add_to_result(b"</form>\n")
    WebOutput.add_to_result((f"<form action='/playlists'>\n").encode("utf-8"))
    WebOutput.add_to_result(b'<label for="playlist">Duplicate playlist:</label>')
    WebOutput.add_to_result((f"<input type='hidden' name='app' value='spotify'>\n").encode("utf-8"))
    WebOutput.add_to_result((f"<input type='hidden' name='action' value='duplicate'>\n").encode("utf-8"))
    WebOutput.add_to_result((f"<input type='hidden' name='playlist' value='{playlist['id']}'>\n").encode("utf-8"))
#                WebOutput.add_to_result((f"<input type='hidden' name='return' value='{request.path}'>\n").encode("utf-8"))
    WebOutput.add_to_result(b"<input type='submit' value='Submit'>")
    WebOutput.add_to_result(b"</form>\n")
    ...
    WebOutput.add_to_result(b"<div class='playlist-view'>")
    WebOutput.add_to_result(b"<div class='playlist-description'>")
    WebOutput.add_to_result((f"{playlist['description']}").encode("utf-8"))
    WebOutput.add_to_result(b"</div>")
    WebOutput.add_to_result(b"<div class='album-tracklist'>")
    WebOutput.add_to_result(b"<table>\n")
    i = 0
