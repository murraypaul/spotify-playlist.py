...
def main():
    ...
    myglobals.SpotifyAPI.trace = True
#    myglobals.SpotifyAPI.trace = False
    ...
    if myglobals.Args.create:
        create_playlist()
    elif myglobals.Args.set_description:
        print("Setting playlist description")
        if myglobals.Args.playlist:
            playlist = get_playlist(myglobals.Args.playlist)
            if playlist:
                myglobals.SpotifyAPI.user_playlist_change_details(user_id,playlist['id'],description=myglobals.Args.set_description)
            else:
                print(f"Playlist {myglobals.Args.playlist} not found")
    elif myglobals.Args.set_image:
        print("Setting playlist image")
        if myglobals.Args.playlist and myglobals.Args.file:
            playlist = get_playlist(myglobals.Args.playlist)
            if playlist:
                with open(myglobals.Args.file,"rb") as image_file:
                    imgbase64 = base64.b64encode(image_file.read())
                myglobals.SpotifyAPI.playlist_upload_cover_image(playlist['id'],imgbase64)
            else:
                print(f"Playlist {myglobals.Args.playlist} not found")
    elif myglobals.Args.find:
        if myglobals.Args.artist and myglobals.Args.album:
            if myglobals.Args.track_name:
                print("Find track")
                album = get_results_for_track(myglobals.Args.track_name,myglobals.Args.artist,myglobals.Args.album,True,False)
            else:
                print("Find album")
                album = get_results_for_album(myglobals.Args.artist,myglobals.Args.album,True,False)
    elif myglobals.Args.add:
        if myglobals.Args.playlist and myglobals.Args.artist and myglobals.Args.album:
            if myglobals.Args.track_name:
                print("Add track (Not implemented)")
            else:
                print("Add album")
                album = find_album(myglobals.Args.artist,myglobals.Args.album)
                playlist = get_playlist(myglobals.Args.playlist)
                if album and playlist:
                    add_album_to_playlist(album,playlist)
    elif myglobals.Args.delete:
        if myglobals.Args.playlist:
            if myglobals.Args.first_albums:
                print("Deleting initial albums from playlist")
                playlist = get_playlist(myglobals.Args.playlist)
                if playlist:
                    process_first_albums(playlist)
            elif myglobals.Args.recent:
                tracks = myglobals.SpotifyAPI.current_user_recently_played()
                process_tracks(tracks['items'])
                while tracks['next']:
                    tracks = myglobals.SpotifyAPI.next(tracks)
                    process_tracks(tracks['items'])
            elif myglobals.Args.all:
                playlist = get_playlist(myglobals.Args.playlist)
                if playlist:
                    if myglobals.Args.dryrun:
                        print("Would delete playlist %s (%s)" % (platlist['name'], playlist['id']))
                    else:
                        myglobals.SpotifyAPI.user_playlist_unfollow(user_id,playlist['id'])
    elif myglobals.Args.list:
            if myglobals.Args.first_albums:
                playlist = get_playlist(myglobals.Args.playlist)
                if playlist:
                ...
                if playlist:
                    ...
                    while tracks['next']:
                        tracks = myglobals.SpotifyAPI.next(tracks)
                        select_duplicate(tracks['items'],'','','',False)
            elif myglobals.Args.recommendations:
                if myglobals.Args.artist:
                    if myglobals.Args.album:
                        if myglobals.Args.track_name:
                            track = get_results_for_track(myglobals.Args.track_name,myglobals.Args.artist,myglobals.Args.album,False,myglobals.Args.interactive)
                            if track and len(track) > 0:
                                tracks = myglobals.SpotifyAPI.recommendations(seed_tracks=[track[0]['id']])
                                process_tracks(tracks['tracks'])
                    else:
                        artist = find_artist(myglobals.Args.artist,True)
                        if artist:
                            tracks = myglobals.SpotifyAPI.recommendations(seed_artists=[artist['id']])
                            process_tracks(tracks['tracks'])

                elif myglobals.Args.genre:
                    tracks = myglobals.SpotifyAPI.recommendations(seed_genres=[myglobals.Args.genre])
                    process_tracks(tracks['tracks'])
                elif myglobals.Args.playlist:
                    playlist = get_playlist(myglobals.Args.playlist)
                    if playlist:
                        recommend_from_playlist(playlist)

    elif myglobals.Args.export:
        export_playlists()
    elif myglobals.Args.query:
        if myglobals.Args.query == "recent":
            query_recent()
        elif myglobals.Args.query == "playlist":
            query_playlist()
    elif myglobals.Args.server:
